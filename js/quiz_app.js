...
function createRadio() {
  ...
  if (i < quiz_questions.length) {
    for (var a = 0; a < 4; a++) {
      options_html +=
        "<input type='radio' name='option' onclick='radio_option_click()' id='option" +
        (a + 1) +
        "'value=''>" +
        " <label for='option" +
        (a + 1) +
        "'>" +
        quiz_questions[i].options[a] +
        "</label><br>" +
        "</br>";
    ...
    for (var d = 0; d < radio.length; d++) {
      if (radio[d].checked) {
        ...
        button_next.onmouseover = function(){
          button_next.style.transition ="0.2s";
          button_next.style.font = "message-box";
        } ;
        mark();
      }
    }
...
/**
 * @description function describes the Mouseover Effect of the
 * next and submit buttons, after options must have been selected
 */
function button_mouseover(){
  button_next.style.transition ="0.2s";
  button_next.style.font = "message-box";
  button_submit.style.transition ="0.2s";
  button_submit.style.font = "message-box";
}
